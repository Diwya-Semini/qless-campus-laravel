        ...
        @foreach($canteens as $canteen)
            <div class="bg-white/[0.02] border border-white/5 rounded-2xl p-6 backdrop-blur-xl shadow-xl relative overflow-hidden group hover:border-white/10 transition-all duration-300">
                ...
                <div class="flex justify-between items-start mb-4">
                    <span class="px-2.5 py-1 text-xs font-black uppercase tracking-wider rounded-md {{ $canteen->is_open == 1 ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20' }}">
                        {{ $canteen->is_open == 1 ? 'Open' : 'Closed' }}
                    </span>
                </div>
